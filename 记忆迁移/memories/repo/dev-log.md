...
## 当前最高优先排查规则（2026-07-24更新）

- 光流/激光/外部传感无数据目前已确认两类互不冲突的硬件/接线故障：其一是异常 UART2 DAP/UART 复合串口模块接入树莓派 USB 后导致 `0x0D` 电池电压数据归零/消失；其二是把 USB-TTL 的 `TX` 接入 `UART5/USART5` 主通信总线，干扰 STM32↔凌霄 IMU 通信。两类故障都会导致外部传感数据异常，并可触发 `[A0 红] 运动解算失效复位`；拔掉异常接线后可能出现 `[A0 绿] 运动解算启动`。
- 以后排查 G_VEL/ALT/`0x33/0x34` 无数据：先查 `0x0D` 电压、供电线、地线、UART2 USB-TTL/DAP-UART 反灌电和 5V/3.3V 电平；同时检查 `UART5/USART5` 是否被外部 TX 接入。`UART5` 若给树莓派做建图数据源，只能 `飞控USART5 RX → USB-TTL RX` 加 GND 旁路监听，USB-TTL TX/VCC 不接。
...
## 2026-07-20：J-Link/OpenOCD 烧录排查结论
...
[2026-07-20] [树莓派真实SLAM cur脚本发送0xF5后未看到UART2 ACK] → [上一阶段固定帧、X/Y/Z方向帧、flags失效帧已由树莓派实测收到`0xA0 GREEN F5 #...`，证明UART2物理链路、F5帧格式、s32小端解析和flags解析曾经正确；当前问题出现在GUI位置测试/F6镜像阶段之后，不能倒推为原始F5方案一直错误] → [本机检查当前源码/ELF：`Uplink_Cmd_Uart2RxByte`、`Uplink_Cmd_Tick`、`Rpi_Position_Mirror_Send`、`s_f5_snapshot`符号存在；当前未提交差异主要为0xF6镜像和GUI位置测试，F5合法帧仍会在`Uplink_Cmd_Dispatch()`内立即通过`DrvUart2SendBuf()`回UART2 0xA0 ACK；Pi日志中的SLAM-cur帧校验正确，`flags=0/1`理论上不应阻止ACK] → [下一步必须做分叉验证：先用上一阶段已通过的固定`send_f5.py`在当前固件上复测ACK；若固定帧仍ACK，问题在新的`send_slam_cur_f5.py`接收线程/过滤/端口占用或SLAM脚本行为；若固定帧也无ACK，再用DAP读`0x20000300`附近的F5快照计数区判断STM32是否解析到F5，从而区分UART2 RX未到、解析失败、还是UART2 TX回传失败；当前本机`lsusb`未见DAP/J-Link，无法直接读RAM] → [安全影响：当前仍只做通信诊断，不接PID、不写`rt_tar`、不触发`0x41`、不改飞行输出；禁止把本问题用回滚或盲目大改解决]
[2026-07-20] [DAP只读RAM确认当前无F5入站记录] → [用户再次运行`send_slam_cur_f5.py`仍无ACK；本机重新识别到工作DAP`5269:6367 ANOTC ANO CMSIS-DAP`] → [通过OpenOCD只读SRAM：`s_f5_snapshot@0x20000300`前48字节全0，按字段解释为cur/tar=0、flags=0、rx_cnt=0、len_err_cnt=0、checksum_err_cnt=0；`s_f5_has_frame@0x20000328`最低字节=0；UART2驱动计数`TxCounter/count/U2RxInCnt/U2RxoutCnt@0x200005d4`均为0] → [结论：当前运行态没有保存任何合法F5，也没有长度/校验失败记录；优先怀疑树莓派`/dev/ttyUSB0`不是当前接到STM32 UART2的那根USB-TTL、TXD→PD6/GND连接松动或接错、USB-TTL电平/端口变化，或者当前脚本/端口只写到了另一个串口设备；下一步不要继续盲发SLAM帧，必须先用上一阶段固定`send_f5.py`复测，再查Pi侧`/dev/serial/by-id`和TX/RX/GND] → [安全影响：OpenOCD仅halt读RAM后resume，未写flash、未reset、未修改控制输出]
[2026-07-20] [树莓派0xF5 ACK恢复，真实SLAM cur阶段进入可观测状态] → [用户回传`send_slam_cur_f5.py`已连续收到`RX 0xA0 GREEN F5 #...`；前段`f=00 c=-2147483648,...`表示SLAM/目标均无效，后段`f=01 c=25,37,0 t=25,37,`表示SLAM有效、目标无效、tar=cur，符合“只发当前位置、不触发控制”的阶段要求] → [DAP只读RAM复核：`s_f5_snapshot`字段解码为6个`INVALID_S32`、`flags=0`、`rx_cnt=140`、`len_err_cnt=0`、`checksum_err_cnt=0`、`has_frame=1`；说明STM32已稳定接收并解析F5，无协议错误。读RAM瞬间最后一帧又回到无效状态，证明SLAM有效性在测试中会切换，后续应重点观察`flags`变化和SLAM定位状态] → [注意：0xA0 ACK文本受`STRING_INFO_MAX_LEN=43`限制，invalid哨兵值会被截断；正常小坐标ACK也可能因显示/脚本日志截断看起来少最后字段，但不影响二进制F5解析。后续若影响调试，可把ACK压缩为更短格式或主要依赖GUI的0xF6二进制镜像] → [安全影响：仍只做通信/观测验证，不接PID、不写`rt_tar`、不触发`0x41`]
[2026-07-20] [GUI位置测试等待0xF6的最终根因确认：只运行ROS导航不会自动发送0xF5，必须运行测试发送程序] → [此前曾从烧录、DAP/J-Link、IMU/数传下行转发、GUI解析等方向排查；用户最终确认当前安全联调设计中，树莓派导航/SLAM节点与0xF5发送程序是解耦的，单独运行导航只建图/定位/导航，不会默认向飞控发送位置帧；只有运行`send_slam_cur_f5.py`等测试程序，STM32才会收到F5并镜像F6] → [用户运行测试程序后，GUI“位置测试”已经能看到原始解析数据；0xF5→STM32→0xF6→数传→GUI链路成立。当前主线从烧录/链路误判切回GUI位置测试功能开发，继续做坐标标定、稳定性测试、轨迹记录和调试诊断；仍不接PID、不发0x41、不改变飞行输出] → [教训：后续联调必须把“ROS导航运行”和“测试发送程序运行”分成两个阶段闸门；排查顺序固定为树莓派TX日志→STM32 ACK→数传F6抓包→GUI页面，不再把没有F6直接归因于烧录或协议错误]
...
[2026-07-26] [GUI新增顶栏rviz快捷入口，方便一键打开ROS2 RViz导航视图] → [用户要求在原GUI顶栏`文件/视图/功能/帮助`旁新增`rviz`按键，点击后后台额外线程运行`rviz2 -d /home/ubuntu22/stm32/ANO_LX_FC/rviz2/rviz/n10p.rviz`，RViz必须作为独立窗口弹出，不嵌入GUI，不影响原串口/位置测试/IMU测试功能；RViz stdout/stderr 要转发到底部日志栏并以紫色`[rviz]`显示；关闭GUI或再次点击rviz必须停止RViz进程，不能留下后台残留] → [实现：新增`gui/services/rviz_process.py`，用`RvizProcessManager`和`_RvizRunner(QThread)`管理外部进程，默认启动前自动尝试`source /opt/ros/humble/setup.bash`，支持`LINGXIAO_RVIZ_COMMAND`注入假命令做自动化测试；修改`gui/main.py`在顶栏加入checkable `rviz`动作，运行时显示`rviz(运行中)`，`closeEvent()`先停止RViz再停串口worker；修改`gui/widgets/log_view.py`使`category=rviz`显示紫色；新增`gui/test/_smoke_rviz.py`，更新`gui/README.md`和`rviz2/use.md`] → [验证：`py_compile`、`git diff --check`、假RViz进程生命周期、GUI关闭清理、offscreen截图均已覆盖；截图见`artifacts/gui/rviz_menu_button.png`和`artifacts/gui/rviz_log_button_running.png`，日志示例包含`[rviz] 启动`、`rviz2 已启动`、假进程输出和`停止 rviz2：GUI 退出`。安全影响：本次仅GUI便利入口，不改飞控固件、不改0xF5/0xF6、不接PID、不发0x41、不改变飞行输出；当前主线仍保持在GUI位置测试/树莓派SLAM对接阶段，RViz只是辅助查看导航地图]
