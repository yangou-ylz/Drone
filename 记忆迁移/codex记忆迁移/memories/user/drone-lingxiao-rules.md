...
15. **无 `0x0D` 电池电压时先别追光流协议** → 本项目已实测：电压帧消失/归零会伴随光流、激光高度、通用速度等外部传感数据消失，并出现 `[A0 红] 运动解算失效复位`；拔掉异常接线后电压和外部传感恢复，并出现 `[A0 绿] 运动解算启动`。以后遇到 G_VEL/ALT/0x33/0x34 无数据，第一步先查电压、供电线、地线、UART2 USB-TTL/DAP-UART 反灌电和 5V/3.3V 电平，同时检查 `UART5/USART5` 是否误接了外部 USB-TTL 的 TX，不要先怀疑光流模块、凌霄 IMU 或 STM32 转发代码。
16. **USART5 只能旁路监听，不能主动接树莓派 TX** → `UART5/USART5` 是 STM32 ↔ 凌霄 IMU 主通信口。板上 `RX/TX` 是相对于 STM32：`RX`=STM32接收IMU数据，`TX`=STM32发送给IMU。树莓派建图只读IMU数据时固定接法是 `飞控USART5 RX → USB-TTL RX`、`GND ↔ GND`，USB-TTL `TX`/`VCC` 和飞控 `USART5 TX` 全部不接。接入 USB-TTL TX 会与IMU/STM32主通信总线冲突，现场已实测会导致电压/光流/激光等数据瞬间异常。
