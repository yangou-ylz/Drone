...
- 典型故障现象：插上某个 DAP/UART 复合串口模块到树莓派 USB 后，即使树莓派关机，`0x0D` 电压也会消失，随后 `0x33/0x34/0x0E` 外部传感显示无数据；拔掉模块后电压和外部传感恢复。
- 因此排查光流/激光无数据时，先确认 `0x0D` 是否正常，再排查 `UART2` USB-TTL/DAP-UART 模块是否造成反灌电、地线参考漂移、5V/3.3V 电平不匹配或串口线异常。
