...
- 光流/激光/外部传感无数据目前已确认两类互不冲突的硬件/接线故障：其一是异常 UART2 DAP/UART 复合串口模块接入树莓派 USB 后导致 `0x0D` 电池电压数据归零/消失；其二是把 USB-TTL 的 `TX` 接入 `UART5/USART5` 主通信总线，干扰 STM32↔凌霄 IMU 通信。两类故障都会导致外部传感数据异常，并可触发 `[A0 红] 运动解算失效复位`；拔掉异常接线后出现 `[A0 绿] 运动解算启动`。
- 旧记录中关于“IMU不采纳0x33/0x34/0x51”“0x34方向字段/0x33触发节奏可能错误”的判断，保留为坏硬件条件下的中间排查记录，但不再作为最终根因。
- 以后排查 G_VEL/ALT/0x33/0x34 无数据：先查 `0x0D` 电压、供电线、地线、UART2 USB-TTL/DAP-UART 反灌电和 5V/3.3V 电平；同时检查 `UART5/USART5` 是否被外部 TX 接入。`UART5` 若给树莓派做建图数据源，只能 `飞控USART5 RX → USB-TTL RX` 加 GND 旁路监听，USB-TTL TX/VCC 不接。完整记录见本文件 2026-07-24 “最终纠错”与“USART5旁路监听”条目。
...
[2026-07-24] [新增独立故障：树莓派用普通USB-TTL接入飞控USART5时，一旦接入USB-TTL TX就会导致电压/外部传感异常；只旁路监听USART5 RX则正常] → [用户进一步实测：树莓派上电后，使用普通USB-TTL模块接入飞控`USART5`和树莓派USB，即使不接VCC，只要按常规串口方式把TX也接入，就会复现“插上马上不行、拔掉马上恢复”；但按只读监听方式连接后完全正常：飞控板`USART5 RX`接USB-TTL `RX`，`GND`接`GND`，飞控`USART5 TX`、USB-TTL `TX`、`VCC/5V`全部不接。用户确认飞控板上`USART5 RX/TX`标注是相对于STM32：`RX`=STM32接收凌霄IMU数据，`TX`=STM32发送数据给凌霄IMU] → [根因：`USART5`不是树莓派通信扩展口，而是STM32↔凌霄IMU的主通信总线。树莓派要拿姿态、速度、高度等IMU数据用于雷达点云建图，本质是在旁路监听`IMU→STM32`这条已有数据线；监听设备只能高阻接收，不能向总线发送。若把USB-TTL `TX`接到飞控`USART5 RX`，等于让USB-TTL和凌霄IMU同时驱动STM32 RX数据线，产生总线争用/乱码/电平钳位；若接到飞控`USART5 TX`，则可能干扰STM32→IMU输入链路。任一情况都可能破坏`0x0D`电池、外部传感、遥控/控制等主数据链路，表现为电压/光流/激光数据瞬间消失] → [解决规则：USART5只允许两种用途：1）STM32内部与凌霄IMU正常通信；2）外部设备只读旁路监听。树莓派建图读取IMU时固定接线为`飞控USART5 RX → USB-TTL RX`、`GND ↔ GND`，USB-TTL `TX`不接、`VCC/5V`不接、飞控`USART5 TX`不接。该接法看似不交叉，但因为是在监听同一条`IMU→STM32`数据线，所以是正确的。严禁把树莓派/USB-TTL作为主动发送端接入USART5；树莓派要主动给飞控发位置/目标，只能走`UART2 0xF5`链路] → [教训：UART引脚标签必须先确认“相对于谁”。普通外设对接常用TX↔RX交叉，但总线旁路监听不是交叉接法；监听已有数据线时，两个接收端可以并联到同一发送端，外部TX必须悬空。此前UART2 DAP/UART硬件问题仍然成立，本条是第二个独立接线错误，两者不冲突；后续任何“插USB串口导致电压/外部传感没了”的排查，都必须同时检查UART2串口桥硬件和UART5是否误接USB-TTL TX。⚠️ 安全影响：USART5是IMU主链路，误接TX可能影响电池、外部传感、遥控、实时控制和IMU状态，不允许带桨/起飞状态下尝试]
