...
- 已实测：异常 ANO `SWD&UART V2.0` 类 DAP/UART 复合模块接到树莓派 USB 后，即使树莓派关机，也会让飞控电压数据归零/消失，并联动导致光流、激光高度、通用速度等外部传感无数据；拔掉后恢复。
- 因此协议排查顺序必须是：电压/供电/地线/串口桥硬件 → `0x0E` 状态 → 树莓派 TX/STM32 ACK → `0x33/0x34/0x51` 字段和转发逻辑。
...
- 光流/激光无数据时，先查 `0x0D` 电压和 UART2 USB-TTL/DAP-UART 模块干扰，再查光流协议。坏串口桥/反灌电导致的电压丢失会伪装成光流或IMU不采纳外部传感。
