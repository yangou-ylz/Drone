...
- 正式对接采用“双端同步阶段闸门”，不是单线开发。
- 每次推进前先确认：当前阶段、STM32 侧已完成内容、树莓派侧已跑通内容、用户回传日志。
- Codex 每次只完成一个可验证小阶段，然后必须告诉用户：本侧完成了什么、树莓派侧下一步跑什么命令、期望看到什么日志、失败时回传哪些材料。
- 未收到树莓派侧该阶段通过结果前，不得继续写后续控制阶段代码；尤其不得跳级接 PID、`0x41` 或实际飞行输出。
- 树莓派侧通过固定帧、三轴方向、flags/失效、SLAM 当前坐标等阶段后，Codex 才依次加入超时保护、误差日志、PID 干运行、低速单轴控制。

官方手册优先：协议字段、地址、模式响应不确定时，查 `用户手册/匿名通信协议V7.pdf` 和 `用户手册/匿名--凌霄--飞控手册.V1.07pdf.pdf`。

硬件前置闸门：

- `0x33/0x34/0x0E` 外部传感异常、GUI显示光流/激光无数据、或出现 `运动解算失效复位` 时，先查 `0x0D` 电池电压是否存在且稳定。
- 已实测两类独立接线/硬件问题会造成同类现象：异常 ANO `SWD&UART V2.0` 类 DAP/UART 复合模块接到树莓派 USB 后会让飞控电压数据归零/消失；以及把 USB-TTL `TX` 接入 `UART5/USART5` 主通信总线会干扰 STM32↔IMU 通信。
- 因此协议排查顺序必须是：电压/供电/地线/串口桥硬件 → 检查 UART5 是否只读监听且无外部TX → `0x0E` 状态 → 树莓派 TX/STM32 ACK → `0x33/0x34/0x51` 字段和转发逻辑。
- `UART5/USART5` 若给树莓派读取IMU数据用于建图，只能接 `飞控USART5 RX → USB-TTL RX` 和 `GND ↔ GND`；USB-TTL `TX/VCC` 不接，飞控 `USART5 TX` 不接。主动控制/位置输入仍走 `UART2 0xF5`。
...
- `0xF5`：树莓派位置帧（cur/tar/flags；当前阶段3a只解析/日志，不控飞）
...
- 匿名光流 V4.0 已建立本地知识页：`记忆迁移/codex记忆迁移/project_docs/sensors/ano-optical-flow-v4.md`；官方 Wiki 原件、正文、图片和下载汇总保存在 `用户手册/匿名光流/`。
- 光流/激光无数据时，先查 `0x0D` 电压、UART2 USB-TTL/DAP-UART 模块干扰、以及 UART5 是否误接外部TX，再查光流协议。坏串口桥、反灌电或主链路总线冲突都会伪装成光流或IMU不采纳外部传感。
