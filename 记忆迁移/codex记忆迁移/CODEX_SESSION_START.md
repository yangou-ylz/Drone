...
5. 外部传感器、光流、激光高度、`0x33/0x34/0x0E` 异常时，先查 `0x0D` 电池电压是否存在且稳定；若插上树莓派 UART2 USB-TTL/DAP-UART 模块后电压归零或出现 `[A0 红] 运动解算失效复位`，优先排查串口桥硬件、反灌电、地线、电平和供电；同时检查 `UART5/USART5` 是否误接了 USB-TTL `TX`。`UART5` 只能旁路监听 `飞控USART5 RX → USB-TTL RX` 和 GND，不要先改光流/IMU协议。
