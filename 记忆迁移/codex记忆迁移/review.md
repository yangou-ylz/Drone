...
- 外部传感/光流/激光高度无数据时，先查 `0x0D` 电池电压、UART2 树莓派串口桥硬件、以及 UART5/USART5 是否误接外部 USB-TTL TX。已实测异常 DAP/UART 复合模块会让电压数据消失；另已实测 UART5 主通信总线一旦接入 USB-TTL TX 也会导致类似异常。UART5 给树莓派建图读取IMU时只能旁路监听 `飞控USART5 RX → USB-TTL RX` 和 GND。
