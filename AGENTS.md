...
- `UART5/USART5` 是 STM32 ↔ 凌霄 IMU 主通信口，不是树莓派可主动发送的调试口。若树莓派只需要读取姿态/速度/高度等 IMU 数据用于雷达建图，只能旁路监听：飞控板 `USART5 RX`（相对 STM32，实际为 IMU→STM32 数据线）接 USB-TTL `RX`，`GND` 接 `GND`；不要接 USB-TTL `TX`，不要接 `VCC/5V`。一旦把 USB-TTL TX 接入这条总线，会和 IMU/STM32 主通信冲突，导致 `0x0D`、光流、激光等数据瞬间异常。
