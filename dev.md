...
### 6.15 2026-07-27 组合面板排版与 GUI 固定轨迹巡航

- 现场日志 `/home/ubuntu22/stm32/ANO_LX_FC/gui/logs/gui_20260727_143454.txt` 已复核：前期预检失败来自 `0x0E` 外部速度/测高无效，传感恢复后重新预检、请求定点、锁定遥控权、一键起飞保持、F9 位移、一键降落均为 `错误=正常(0x0000)`。F9 示例 `seq=2` 执行 `X=50cm, Y=-50cm, Z=0`，进入 `位移到位保持`；随后一键降落完成并上锁。
- 组合面板排版调整：有参数的小窗口统一为“顶部安全确认、左侧触发按钮、竖线分隔、右侧参数设置”。这样避免“起飞高度”的输入框和“总超时”标签过近造成歧义。飞前准备这类纯按钮区仍保持正常网格排放。
- 新增 GUI 侧固定轨迹巡航，不新增飞控帧、不改 STM32 状态机、不改 PID3D。巡航只是由 GUI 串行发送已验证的 `0xF9` 单次位移命令。
- 巡航轨迹第一版：矩形闭环 `前→左→后→右`、正方形闭环、直角 `前→左`、直角 `前→右`、前后往返。参数为前后距离、左右距离、段间等待，均可在 GUI 调整。
- 分段完成判断：每段发送 F9 START 时记录该段 `seq`；GUI 只在收到 F8 且 `last_cmd==0xF9`、`last_cmd_seq==当前段seq`、`state==MOVE_HOLD(24)`、`error==0` 时，等待设定的段间时间后发送下一段。这样不会被旧的到位状态误触发。
- 安全处理：任一 F9 回执 warning/fail/timeout、F8 error 非零、用户点击停止巡航、手动 F9 STOP、F7 一键降落/中止/急停，都会取消后续巡航段；“停止巡航”还会同时发送 F9 STOP 清零位移速度输出。
- 目的：给雷达/SLAM 调参提供可重复的闭环相对运动轨迹。当前仍以飞控速度积分 PID3D 为控制来源，树莓派 `0xF5` 仅观察，不参与位置环。

### 6.16 2026-07-27 固定轨迹巡航通过与键盘低速速度控制

- 现场日志 `/home/ubuntu22/stm32/ANO_LX_FC/gui/logs/gui_20260727_161403.txt` 已复核，用户现场观察也确认无人机巡航飞行正常。
- 巡航表现：起飞保持后，连续 F9 分段均按 `开始位移 → 位移运行 → 位移到位保持` 完成；示例 `seq=1~4` 完成 50cm 矩形闭环，`seq=6~9` 再次完成 50cm 闭环；全程 F8 关键状态均为 `错误=正常(0x0000)`，电压负载约 `15.25~15.63V`，外部速度/测高正常。
- 发现并修正小漏洞：F7/F9 原先共用 `s_last_cmd_seq` 去重，导致 F9 STOP `seq=5` 后，F7 LAND `seq=5` 被误判为重复。现已改为 F7/F9/FA 分别维护最近 seq，F8 的 `last_cmd_seq` 仍用于显示最近命令。
- 新增 `0xFA` GUI 键盘低速速度命令：`AA FF FA 0E | ver seq cmd key vx vy yaw flags | SC AC`。只控制 `vel_x/vel_y/yaw_dps`，绝不写 roll/pitch 角速度；`rol/pit/thr/vel_z` 固定清零。
- 控制语义：按住键盘才持续发速度，松开立即发零速；GUI 每 100ms 发送当前按键合成速度，固件 300ms 未收到新速度帧自动清零退出，防止 GUI 卡死后继续飞。
- 优先级：FA SET 会停止 GUI PID3D/F9 位移并接管 `rt_tar`，用于巡航/位移时紧急低速修正；如果 GUI 已释放遥控权，则 FA 被拒绝，避免遥控器和 GUI 速度控制打架。
- 限幅：STM32 侧强制 `|vx|/|vy| <= 30cm/s`、`|yaw_dps| <= 45deg/s`；GUI 默认线速度 `15cm/s`、角速度 `20deg/s`，可调但不能超过固件上限。对角线运动 GUI 做 0.707 归一化，避免合速度超过单轴量级。
- GUI 新增“键盘低速速度控制”小窗口：左侧显示二维摇杆和 yaw 滑块动画，右侧是启用/关闭、按键校准、查询、线速度/角速度设置和实时数值。按键校准按顺序捕获上、下、左、右、A、D 的实际 Qt key 值，降低键位映射错误风险。
- 按键捕获补充：新增独立脚本 `gui/tools/capture_velocity_keys.py`，运行后会打开一个小 Qt 窗口，并在终端依次提示按“向上/向下/向左/向右/A/D”。捕获到的是 GUI 实际使用的 Qt key code，保存到 `gui/keymaps/velocity_keys.json`；主 GUI 启动时自动读取。GUI 内部“按键校准”也会写入同一文件。
- 带宽策略：高频 FA SET 静默发送，不登记 ACK、不刷 GUI 发送日志；查询/停止保留正常 ACK。固件只在进入速度模式、停止、超时、拒绝/错误时发关键 A0，不按 10Hz 刷速度回执。

### 6.17 2026-07-27 恢复树莓派位置测试主线与雷达质量报告

- 当前主线恢复为：树莓派 `0xF5` 位置帧只读观测 → STM32 解析并镜像 `0xF6` → GUI“位置测试”分析雷达/SLAM 数据质量。F7 一键起飞、F9 相对位移、固定巡航、FA 键盘速度控制只作为可重复运动源，不能被误解为已经接入树莓派位置闭环。
- 纪律重申：当前阶段不让 `0xF5` 写 `rt_tar`，不触发 `0x41`，不接位置 PID，不在 STM32 内临时修树莓派坐标转换。树莓派位置数据只有在静止稳定、坐标方向/比例、动态连续性、闭环轨迹误差和延时都通过后，才进入后续“干运行位置环”讨论。
- GUI 位置测试新增“雷达报告”页：点击“开始会话记录”后，后台持续记录 `0xF6` 雷达位置、`0xF8` 自主状态和 GUI 实际发出的 F7/F9/FA 命令事件；即使切到自主控制页执行起飞、位移、巡航，也不会停止记录，直到用户手动停止。
- 雷达报告页实时显示：记录时长、F6 样本数、SLAM 有效率、F5输入/F6镜像频率、长度/校验错误增量、XY/Z 抖动、XY 漂移、跳变数量、最新 F8 状态/错误、F9 段误差。
- 轨迹对比：记录 F9 START 时以最近有效 `0xF6 cur` 作为起点，按 GUI 位移命令叠加期望段；收到对应 `F8 last_cmd=0xF9, state=MOVE_HOLD, error=0` 后，用 1 秒内的新鲜有效 `cur` 计算段误差；若 F6 停发或过旧，则该段不强行给出误差，避免旧坐标误判。该误差只用于评价雷达观测，不代表飞控使用了树莓派位置。
- 导出能力：停止记录后可导出会话 CSV 和文本质量报告，用于和树莓派日志、现场现象一起复盘。CSV 混合记录 F6/F8/CMD 事件，便于后续做延时估计和轨迹对齐。
- 导出修复：雷达报告 CSV/文本报告、轨迹 CSV 不再使用系统原生 `QFileDialog.getSaveFileName()`，改为 Qt 非原生保存对话框，默认目录为 `gui/logs/position_test_exports/`；同时加入写文件异常弹窗，避免 Ubuntu 桌面门户/原生文件框卡住时表现成“点击无反应”。
- 已验证：`py_compile gui/position_test/position_test_window.py gui/main.py` 通过；离线注入模拟 F6/F8/F9 后，后台记录、F9 段误差计算和 CSV 导出通过；`git diff --check` 通过。

### 6.18 2026-07-27 雷达联调临时工作台

- 新增位置测试内的“雷达联调”页，并在主菜单栏增加顶层“雷达联调”快捷按钮。点击后直接打开该页，减少现场反复进入功能菜单、切页面、滚动到底部的操作。
- 该页面只做当前雷达/SLAM 调参循环需要的最小集成：开始/停止记录、停止并导出 CSV+报告、查询状态、预检、请求定点、锁定遥控权、一键起飞保持、正方形/L型/T型/斜线巡航、停止巡航、一键降落、中止降落、强制上锁。
- 状态只显示关键字段：记录计数、F5/F6频率、SLAM有效性、cur坐标、电压、Mode/解锁、F8状态/错误、外部速度/测高/F5年龄、巡航进度。避免加入大量无关调试控件。
- 正方形巡航仍然只是 GUI 串行发送已验证的 F9 相对位移段，按 F8 `last_cmd=0xF9 + state=MOVE_HOLD + error=0` 自动切下一段；不新增飞控协议、不改 STM32 状态机、不接树莓派位置闭环。
- 巡航轨迹扩展：L型=X后Y同距离；T型=X一次后Y左再Y右穿过中心；斜线型=XY同时一段并使用 `AUTO_MOVE_AXIS_XY`；正方形仍为前、左、后、右闭环。

- 一键起飞保持和正方形巡航会自动启动雷达会话记录；“停止并导出CSV+报告”会停止记录并自动写入 `gui/logs/position_test_exports/`，不弹保存框，减少现场操作负担。
- 已验证：`py_compile gui/position_test/position_test_window.py gui/main.py` 通过；离线模拟雷达联调页起飞、F9正方形首段、F8到位切下一段、CSV+报告导出通过；离线校验正方形/L型/T型/斜线型发送段参数通过；`git diff --check` 通过。

### 6.19 2026-07-28 雷达联调页布局防撑爆修复

- 现场问题：雷达联调页开始/停止记录或导出后，页面会突然变得很大并冲出屏幕，且鼠标无法滚动到底部。根因是页面没有滚动容器，且底部状态文本/导出路径和关键状态值会参与 Qt 最小尺寸计算，把整个主窗口撑大。
- 修复：雷达联调页整体放入 `QScrollArea`，禁用横向滚动并允许纵向滚动；标题、提示、关键状态值和底部状态栏设置 `Ignored/Preferred` 尺寸策略和最大高度，避免文本撑大窗口。
- 修复：导出后的状态栏不再显示完整绝对路径，只显示短路径，完整 CSV/报告路径放入 tooltip；底层“雷达报告”页导出状态同样改为短路径显示。
- 已验证：离线 GUI 测试中开始记录、停止记录、导出前后雷达联调页 `sizeHint` 宽度保持不变；四种巡航轨迹发送参数回归通过；`py_compile` 和 `git diff --check` 通过。
