...
//
#define MAX_VELOCITY 25	  // 最大打杆时水平速度，单位厘米每秒（满杆=25cm/s，摇杆动一半~12cm/s）
...
static inline void RC_Data_Task(float dT_s)
{
	...
	if (rc_in.fail_safe == 0)
	{
		...
		// 摇杆数据转换到+-500并加死区
		float tmp_ch_dz[4];
		tmp_ch_dz[ch_1_rol] = my_deadzone((rc_in.rc_ch.st_data.ch_[ch_1_rol] - 1500), 0, 80);
		tmp_ch_dz[ch_2_pit] = my_deadzone((rc_in.rc_ch.st_data.ch_[ch_2_pit] - 1500), 0, 80);
		...
		// 摇杆转换姿态量、油门量
		// 比例系数0.00238f = 1/(500-80)，补偿死区后保证满杆=1.0（原本0.002f=1/500无死区，CH1/CH2死区80，CH4死区80）。
		// 注意正负号需要满足ANO坐标系定义，一般情况姿态角表示方向（包括上位机）和遥控摇杆反向都与飞控使用的ANO坐标系不同。
		//		if(mod_f[0]<2)
		//		{

		rt_tar.st_data.rol = tmp_ch_dz[ch_1_rol] * 0.00238f * MAX_ANGLE;
		rt_tar.st_data.pit = tmp_ch_dz[ch_2_pit] * 0.00238f * MAX_ANGLE;		// 摇杆前推通道增大，与ANO坐标系同向，不取负
		...
		// 实时XYZ-YAW期望速度(实时控制帧)
		// 模式2(定点)下启用遥控速度控制：
		// CH1(roll) -> vel_x，CH2(pitch) -> vel_y，CH4(yaw) 保持原有 yaw_dps 处理。
		if (mod_f[0] == 2)
		{
			// 任一PID任务触发期间（CH6/CH10/CH7高档）由UserTask写入速度目标，
			// 这里不再覆盖，避免100Hz RC任务与50Hz PID任务抢写导致响应变慢。
			u8 pid_task_xy_active = 0;
			u8 pid_task_z_active = 0;
			if ((rc_in.rc_ch.st_data.ch_[ch_6_aux2] > 1700 && rc_in.rc_ch.st_data.ch_[ch_6_aux2] < 2200) ||
				(rc_in.rc_ch.st_data.ch_[ch_10_aux6] > 1700 && rc_in.rc_ch.st_data.ch_[ch_10_aux6] < 2200))
			{
				pid_task_xy_active = 1;
			}
			...
			if (pid_task_xy_active)
			{
				// X/Y任务不改高度，固定中位油门以降低高度漂移。
				rt_tar.st_data.thr = 500;
			}
			else if (pid_task_z_active)
			{
				// Z任务本身控制高度偏移，不固定油门，避免逻辑冲突。
			}
			else
			{
